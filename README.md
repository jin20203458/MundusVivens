...
```mermaid
graph TD
    subgraph C++ Game Server [육체 (Body) & 물리 월드]
        Physics[20Hz Lock-Free Tick]
        ECS[EnTT ECS System]
        Spatial[Spatial Hash Grid & A* Pathfinding]
        Proximity[공간 인접 및 대화 발화 감지]
    end

    subgraph Neural Net [신경계 (gRPC Pipeline)]
        gRPC[mundus_vivens.proto / Bidirectional Stream]
    end

    subgraph C# AI Server [뇌 (Brain) & 인지 엔진]
        Cognition[BeliefEngine & MemoryBox]
        Social[Relationship Graph & Emotion]
        LLMOrchestration[Gemini LLM API & DailyPlan]
    end
    ...
    LLMOrchestration -->|2. 대사/행동 추론 결과 내려줌| gRPC
    gRPC --> ECS
```
